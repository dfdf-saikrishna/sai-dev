...
class Masteradmin_List_Table extends \WP_List_Table
{
    ...
    function column_name($item)
    {
        ...
        $actions = array(
            'edit' => sprintf('<a href="?page=companiesmenu" data-id=%s>%s</a>', $item['SUP_Id'], __('Edit', 'masteradmin_table_list')),
            'delete' => sprintf('<a href="?page=%s&action=delete&id=%s">%s</a>', $_REQUEST['page'], $item['SUP_Id'], __('Delete', 'masteradmin_table_list')),
        );
		return sprintf('%s %s %s',
            '<a href="'.erp_company_url_single_masteradminview( $item['SUP_Id']).'"><strong>' . $item['SUP_Name'] . '</strong></a>',
    ...
    function prepare_items()
    {
		...
		//$selcom="SUP_Id>1";	  
		//$selcom.=" AND SUP_Status=1 AND SUP_Type=2";

        $counttotal_items = $wpdb->get_var("SELECT * FROM $table_name WHERE '". $selcom ."'");
		...
		if(!empty($_POST["s"])) {
			...
			$this->items = $wpdb->get_results($wpdb->prepare("SELECT * FROM $table_name WHERE '". $selcom ."'".$query."ORDER BY $orderby $order LIMIT %d OFFSET %d", $per_page, $paged), ARRAY_A);
		}
		else{
			$this->items = $wpdb->get_results($wpdb->prepare("SELECT * FROM $table_name WHERE  '". $selcom ."' ORDER BY $orderby $order LIMIT %d OFFSET %d", $per_page, $paged), ARRAY_A);
		}
