        ...
        */
        ...
        function prepare_items() {
            ...
            if (!empty($_POST["s"])) {
                ...
                $this->items = $wpdb->get_results($wpdb->prepare("SELECT DISTINCT (req.REQ_Id) AS reqids, req.*,emp.* FROM requests req, request_employee re, employees emp WHERE req.COM_Id='$compid' AND req.REQ_Id=re.REQ_Id AND re.EMP_Id != $mydetails->EMP_Id AND re.EMP_Id = emp.EMP_Id AND req.REQ_Active !=9 AND re.RE_Status=1 " . $query . " ORDER BY $orderby $order LIMIT %d OFFSET %d", $per_page, $paged), ARRAY_A);
            }
