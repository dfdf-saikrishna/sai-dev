...
//        );
         $actions = array(
            'edit' => sprintf('<a href="?page=persons_form&id=%s">%s</a>', $item['EMP_Id'], __('Edit', 'custom_table_example')),
            'delete' => sprintf('<a href="?page=%s&action=delete&id=%s">%s</a>', $_REQUEST['page'], $item['EMP_Id'], __('Delete', 'custom_table_example')),
        );

        return sprintf('%s %s',
            $item['EMP_Name']. ',</br>' . $item['EMP_Code'],
...
//            return $actions;
    ...
    function prepare_items() {
        ...
        // print_r($total_items/$per_page);die;
        ...
        //print_r($comId);die;
       $this->items = $wpdb->get_results($wpdb->prepare("SELECT * FROM approval_limit al, employees emp WHERE al.APL_Active=1 AND al.EMP_Id = emp.EMP_Id AND emp.COM_Id='$COM_Id' AND EMP_Status=1 AND EMP_AccountsApprover=1 AND EMP_Access=1 ORDER BY $orderby $order LIMIT %d OFFSET %d", $per_page, $paged), ARRAY_A);
